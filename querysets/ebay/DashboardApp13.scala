// Get the average sales per month in each year during the selected range for the seller
class DashboardApp13(paramMap: Map[String, Any]) extends Serializable {
  ...
  val reviewScore = paramMap("reviewScore").asInstanceOf[Float]
    ...
    val row = data.asInstanceOf[RowType]
		((row._3.toFloat * 24 * 60 * 60 / (60 * 60 * 24 * 365)).toLong, row._7.toFloat)
	}
