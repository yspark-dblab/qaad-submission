// Get the averages sales minute in each hour during the selected range for the seller
class DashboardApp10(paramMap: Map[String, Any]) extends Serializable {
  import QaaD._
  ...
  val reviewScore = paramMap("reviewScore").asInstanceOf[Float]
    ...
    val row = data.asInstanceOf[RowType]
		((row._3.toFloat * 24 * 60 * 60 / (60 * 60)).toLong, row._7.toFloat)
	}
