// Get the average number of orders per week in each month during the selected range for the seller
class DashboardApp17(paramMap: Map[String, Any]) extends Serializable {
  import QaaD._
  ...
  val reviewScore = paramMap("reviewScore").asInstanceOf[Float]
    ...
    val row = data.asInstanceOf[RowType]
		((row._3.toFloat * 24 * 60 * 60 / (60 * 60 * 24 * 30)).toLong, 1L)
	}
