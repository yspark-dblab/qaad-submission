// Get the nubmer of reviews per year during the given time range for the seller
class DashboardApp31(paramMap: Map[String, Any]) extends Serializable {
  import QaaD._
  ...
  val reviewScore = paramMap("reviewScore").asInstanceOf[Float]
    ...
    val row = data.asInstanceOf[RowType]
		((row._3.toFloat * 24 * 60 * 60 / (60 * 60 * 24 * 365)).toLong, 1L)
	}
