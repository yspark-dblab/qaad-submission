// Get the number of ordered units per hour during the given time range for the seller
class DashboardApp6(paramMap: Map[String, Any]) extends Serializable {
  import QaaD._
  ...
  type OrderReviewRow = (String, String, String, String, String) // review_id, order_id, review_score, review_creation_date, review_answer_timestamp
	...
	def f2(data: Any): (String, Int) = { // map
    val row = data.asInstanceOf[OrderRow]
		(row._1, row._4.toInt)
	}
	...
	def f5(data: Any): (Int, Int) = {
    val row = data.asInstanceOf[(String, (Int, String))]
    (row._2._1 / (60 * 60), 1)
	}
