// Get product, price and review score given a period for a seller (product catalog)

class DashboardApp26(paramMap: Map[String, Any]) extends Serializable {
  import QaaD._
  ...
  val numRows = paramMap("numRows").asInstanceOf[Int]
	...
	def f4(data: Any): (String, (String, Float)) = {
		val row = data.asInstanceOf[(String, ((String, Float), String))]
		(row._1, row._2._1)
	}
	...
	def f6(data: Any): (String, Float, Float) = {
		val row = data.asInstanceOf[(String, ((String, Float), Float))]
