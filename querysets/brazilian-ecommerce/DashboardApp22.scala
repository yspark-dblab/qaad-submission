// Get number of ordered items per category in a particular period

class DashboardApp22(paramMap: Map[String, Any]) extends Serializable {
  import QaaD._
  ...
  val numRows = paramMap("numRows").asInstanceOf[Int]
	...
	def f3(data: Any): (String, (String, String)) = {
		val row = data.asInstanceOf[OrderItemRow]
		(row._1, (row._3, row._2))
	}

	def f4(data: Any): (String, String) = {
		val row = data.asInstanceOf[(String, (String, (String, String)))]
		row._2._2
	}
  ...
  def run(): Unit = {
    ...
    rddOrders.qFilter(x => f1(x))
      .qMap(x => f2(x))
      .qJoin(rddOrderItems.qMap(x => f3(x)))
      .qMap(x => f4(x))
      .qJoin(rddProducts.qMap(x => f5(x)))
      .qMap(x => f6(x))
  ...
  def seqRun(rddOrders: RDD[Any],
    ...
    rddOrders.filter(x => f1(x))
      .map(x => f2(x))
      .join(rddOrderItems.map(x => f3(x)))
      .map(x => f4(x))
      .join(rddProducts.map(x => f5(x)))
      .map(x => f6(x))
      .reduceByKey((x, y) => x + y)
