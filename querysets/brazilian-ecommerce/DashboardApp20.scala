// Get the number of reviews of which score is less than a particular review score for a specific seller

class DashboardApp20(paramMap: Map[String, Any]) extends Serializable {
  import QaaD._
  ...
  val numRows = paramMap("numRows").asInstanceOf[Int]
	...
	def f5(data: Any): Boolean = {
		val row = data.asInstanceOf[OrderReviewRow]
	  row._3.toFloat < reviewScore
	}

	def f6(data: Any): (String, String) = {
		val row = data.asInstanceOf[OrderReviewRow]
		(row._2, row._1)
	}

	def f7(data: Any): (Int, Int) = {
		val row = data.asInstanceOf[(String, (String, String))]
		(0, 1)
	}

	def f8(data1: Any, data2: Any): (Int, Int) = {
  ...
  def f9(data: Any): Int = {
    val row = data.asInstanceOf[(Int, Int)]
    row._2
  }
  ...
  def run(): Unit = {
    ...
    rddOrderItems.qFilter(x => f1(x))
      ...
      .qMap(x => f4(x))
      .qJoin(rddOrderReviews.qFilter(x => f5(x))
        .qMap(x => f6(x)))
      .qMap(x => f7(x))
      .qReduceByKey((x, y) => f8(x, y))
      .qMap(x => f9(x))
  }
  ...
  def seqRun(rddOrders: RDD[Any],
    ...
    rddOrderItems.filter(x => f1(x))
      ...
      .map(x => f4(x))
      .join(rddOrderReviews.filter(x => f5(x))
        .map(x => f6(x)))
      .map(x => f7(x))
      .reduceByKey((x, y) => x + y)
      .map(x => f9(x))
