// Get the average number of ordered units per week in each month during the given time range for the seller
class DashboardApp17(paramMap: Map[String, Any]) extends Serializable {
  ...
  def f7(data: Any): (Long, Float) = {
    ...
    (row._1, row._2.toFloat / (60 * 60 * 24 * 7))
  }
