// Get recent 5 product review scores for a specific seller
...
class DashboardApp24(paramMap: Map[String, Any]) extends Serializable {
  ...
  val numRows = paramMap("numRows").asInstanceOf[Int]
	...
	def f5(data: Any): (String, (Float, Long)) = {
		val row = data.asInstanceOf[OrderReviewRow]
		(row._2, (row._3.toFloat, row._4.toLong))
	}

	def f6(data: Any): (Int, Seq[(Float, Long)]) = {
		val row = data.asInstanceOf[(String, (String, (Float, Long)))]
		(0, Seq(row._2._2))
	}

	def f7(data1: Any, data2: Any): (Int, Seq[(Float, Long)]) = {
		val row1 = data1.asInstanceOf[(Int, Seq[(Float, Long)])]
		val row2 = data2.asInstanceOf[(Int, Seq[(Float, Long)])]
		(row1._1, row1._2 ++ row2._2)
	}

  def f8(data: Any): Seq[Int] = {
    val row = data.asInstanceOf[(Int, Seq[(Float, Long)])]
    row._2.sortBy(-_._2).slice(0, scala.math.min(5, row._2.size)).map(_._1.toInt)
  }
